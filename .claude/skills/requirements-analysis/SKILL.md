...
description:
  제공된 요구사항을 분석하고, 개발자와의 질의응답을 통해 구체적이지 않은 요구사항을 명확히 하여 정리합니다.
  모든 정리가 끝나면, 시퀀스 다이어그램, 클래스 다이어그램, ERD 등을 Mermaid 문법으로 작성합니다.
---

요구사항을 분석할 때 반드시 다음 흐름을 따른다.

### 1. 요구사항을 재해석하고, 요구사항이 필요한 이유를 설명한다.
- 요구사항 문장을 정리하는 데서 끝내지 않는다.
- "무엇을 만들까?"가 아니라 "지금 어떤 상황이고 해결해야하는 과제가 무엇인지, 왜 해결해야하는지?" 확인한다.
- 다음 3가지 관점에서 정리한다.
    - 사용자 관점
    - 비즈니스 관점
    - 시스템 관점
...
### 3. 요구사항 명확화를 위한 질문을 개발자가 답변하기 쉬운 형태로 제공한다.
...
### 4. 3단계 까지 정리된 내용을 바탕으로 개념 모델을 정의한다.
...
### 5. 다이어그램은 항상 이유 → 다이어그램 → 해석 순서로 제시한다.
...
### 6. 다이어그램 작성 완료 후 읽는 법을 짚어준다.
- 이 구조에서 주의깊게 봐야 하는 포인트를 2~3줄로 설명한다.
- 설계 의도가 드러나도록 해석을 붙인다.

### 7. 설계 완료 후 남아 있는 잠재적 리스크를 반드시 언급한다.
