...
**메서드명에 구현 상세를 넣지 않는다.**
구현 상세란 구현 메서드명(`validateUsable`, `calculateDiscount`), 예외 클래스명(`CoreException`), ErrorType(`NOT_FOUND`, `CONFLICT`), 컬럼명(`deletedAt`), 엔티티명 등을 말한다.
...
| `CoreException_NOT_FOUND가_발생한다` | `예외가_발생한다` | 예외 클래스·ErrorType은 assertion에서 검증 |
...
| 레이어 | 기대결과 표현 | 예시 |
|---|---|---|
| **Domain** | 엔티티 상태 변화 | `쿠폰_생성_시_쿠폰이_생성된다`, `삭제_시_삭제_처리된다` |
...
| `상품을_삭제하면_성공한다` | `상품_삭제_시_삭제_처리된다` | 상태 변화를 명시 |
...
3. 테스트 메서드명에 "성공한다", "실패한다" 같은 모호한 표현, 구현 상세(예외 클래스명, ErrorType, 컬럼명)를 사용하지 않는다.
