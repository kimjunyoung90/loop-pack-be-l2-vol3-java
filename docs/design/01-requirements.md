...
| 용어 | 영문 | 정의 |
|------|------|------|
| 브랜드 | Brand | 상품을 판매하는 주체. 이름과 설명을 가진다 |
...
| 용어 | 영문 | 정의 |
|------|------|------|
| 상품 | Product | 판매 가능한 항목. 이름, 가격, 설명을 가진다 |
...
컨텍스트별로 지켜야 하는 도메인 규칙과 운영 정책을 정의한다.
...
- 브랜드 삭제 시 해당 브랜드에 속한 상품과 해당 상품의 좋아요도 함께 삭제된다.

### 상품 컨텍스트

- 상품 등록 시 해당 상품이 속할 브랜드가 사전에 등록되어 있어야 한다.
- 상품 삭제 시 해당 상품의 좋아요도 함께 삭제된다.
...
- 주문 시 하나라도 재고가 부족한 상품이 있으면 주문 전체가 실패한다. (All or Nothing)
- 주문 생성 시 주문 시점의 상품 정보(상품명, 단가)를 스냅샷으로 저장한다.
...
| 컨텍스트 | 역할 | 기능 |
|----------|------|------|
| 브랜드 | 일반 사용자 | 목록 조회, 상세 조회 |
| 브랜드 | 관리자 | 등록, 목록 조회, 상세 조회, 수정, 삭제 |
| 상품 | 일반 사용자 | 목록 조회, 상세 조회 |
| 상품 | 관리자 | 등록, 목록 조회, 상세 조회, 수정, 삭제 |
| 좋아요 | 일반 사용자 | 등록, 취소, 좋아요한 상품 목록 조회 |
| 주문 | 일반 사용자 | 생성, 취소, 목록 조회, 상세 조회 |
| 주문 | 관리자 | 목록 조회, 상세 조회 |
...
- 사용자는 최신 등록순으로 브랜드 목록을 정렬할 수 있다.
- 사용자는 좋아요가 많은 순으로 브랜드 목록을 정렬할 수 있다.
...
- 사용자가 좋아요가 많은 순으로 보기를 원하면 좋아요가 많은 순으로 정렬해 보여준다.
...
1. 관리자 여부를 확인한다.
2. 브랜드 존재 여부를 확인한다.
3. 해당 브랜드에 속한 상품의 좋아요를 전부 삭제한다.
4. 해당 브랜드에 속한 상품을 전부 삭제한다.
5. 브랜드를 삭제한다.
...
- 사용자는 최신 등록순으로 상품 목록을 정렬할 수 있다.
- 사용자는 가격순(오름차순/내림차순)으로 상품 목록을 정렬할 수 있다.
- 사용자는 좋아요가 많은 순으로 상품 목록을 정렬할 수 있다.
- 사용자는 특정 브랜드의 상품만 필터링하여 조회할 수 있다.
...
1. 관리자 여부를 확인한다.
2. 상품이 속할 브랜드의 존재 여부를 확인한다.
3. 상품 정보를 등록한다. (초기 재고 수량 포함)
4. 등록된 상품 정보를 반환한다.
...
1. 로그인 여부를 확인한다.
2. 해당 상품에 대한 좋아요 여부를 확인한다.
3. 좋아요를 삭제한다.
4. 상품의 좋아요 수를 감소시킨다. (비동기)
...
1. 로그인 여부를 확인한다.
2. 전체 주문 상품의 존재 여부를 확인한다.
3. 주문 상품마다 재고를 검증한다.
  - 하나라도 실패 시 전체 롤백 (All or Nothing)
4. 주문을 생성한다.
  - 주문 시점의 상품 정보(상품명, 단가)를 스냅샷으로 저장한다.
5. 재고를 차감한다.
6. 주문 완료 응답을 반환한다.
...
- 사용자는 기간별(주문일 기준)로 주문 내역을 조회할 수 있다.
