...
| PAY-03 | 이미 최종 처리된 결제(SUCCESS, FAILED)는 상태를 변경할 수 없다. |
...
- PG 서버가 비정상 응답을 반환하면 결제를 FAILED 상태로 변경한다.
- PG 서버에 타임아웃이 발생하면 결제를 UNKNOWN 상태로 변경한다.
- 서킷 브레이커가 OPEN 상태이면 PG 요청 없이 결제를 FAILED 상태로 변경한다.
...
- 이미 최종 처리된 결제(SUCCESS, FAILED)인 경우 요청에 실패한다.
