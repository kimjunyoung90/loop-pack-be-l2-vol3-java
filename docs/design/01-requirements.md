...
5. 사용자 쿠폰이 적용된 경우 쿠폰을 검증하고 할인 금액을 계산한다.
   - 사용자 쿠폰의 존재 여부를 확인한다.
   - 본인 소유의 쿠폰인지 확인한다.
   - 쿠폰의 사용 가능 상태를 확인한다. (사용 완료/만료 여부)
   - 주문 금액이 쿠폰의 최소 주문 금액 이상인지 확인한다.
   - 할인 금액을 계산한다. (정액 할인이 주문 금액을 초과하면 최종 결제 금액은 0원)
6. 주문을 생성한다. (쿠폰 적용 전 금액, 할인 금액, 최종 결제 금액 포함)
7. 적용된 사용자 쿠폰을 사용 완료 상태로 변경한다.
8. 주문 완료 응답을 반환한다.
...
4. 주문을 취소한다.
5. 주문 상품별 재고를 복원한다.
6. 적용된 사용자 쿠폰이 있는 경우 사용 가능 상태로 복원한다.
