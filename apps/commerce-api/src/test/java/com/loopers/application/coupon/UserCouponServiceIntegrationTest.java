...
class UserCouponServiceIntegrationTest {
    ...
    void 사용자_쿠폰_생성_시_쿠폰_정보가_DB에_저장된다() {
        ...
        // when
        couponService.issueCoupon(1L, couponResult.id());

        // then - DB에서 조회하여 실제 저장 확인
        List<UserCouponResult> savedCoupons = couponService.getUserCoupons(1L);
        assertThat(savedCoupons).hasSize(1);

        UserCouponResult saved = savedCoupons.getFirst();
        assertThat(saved.userId()).isEqualTo(1L);
        assertThat(saved.couponId()).isEqualTo(couponResult.id());
    ...
    void 쿠폰_목록_조회_시_해당_사용자의_쿠폰만_조회된다() {
        ...
        // when
        List<UserCouponResult> result = couponService.getUserCoupons(1L);
    ...
    void 쿠폰별_발급_내역_조회_시_해당_쿠폰의_발급_내역만_페이징_조회된다() {
        ...
        assertThat(result.getContent()).allMatch(r -> r.couponId().equals(coupon1.id()));
    }
