...
class OrderConcurrencyIntegrationTest {
            ...
            executorService.submit(() -> {
                try {
                    ...
                    CreateOrderCommand command = new CreateOrderCommand(user.id(), null, List.of(
                            new CreateOrderCommand.CreateOrderItemCommand(product.id(), quantityPerOrder)
                    ));
    ...
    void 여러_상품_주문_시_하나라도_재고가_부족하면_주문이_실패하고_모든_재고가_롤백된다() throws InterruptedException {
        // given
        BrandInfo brand = brandService.createBrand(new CreateBrandCommand("아디다스"));
        int productAStock = 100;
        int productBStock = 5;
        ProductInfo productA = productService.createProduct(brand.id(),
                new CreateProductCommand(brand.id(), "운동화", 50000, productAStock));
        ProductInfo productB = productService.createProduct(brand.id(),
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            executorService.submit(() -> {
                try {
                    ...
                    CreateOrderCommand command = new CreateOrderCommand(user.id(), null, List.of(
                            new CreateOrderCommand.CreateOrderItemCommand(productA.id(), 1),
                            new CreateOrderCommand.CreateOrderItemCommand(productB.id(), 1)
                    ));
                    orderFacade.createOrder(command);
                    successCount.incrementAndGet();
                } catch (Exception e) {
